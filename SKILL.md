---
name: xxd-panel-121
description: "Create XXD Panel 121 raster artwork from photographs using naïve lifestyle doodles, relaxed marker drawing, mature invisible-grid composition, intelligent handwritten text relationships, and exceptionally abundant whitespace. Use when the user invokes xxd-panel-121 or requests this restrained lifestyle editorial style. Supports individual images, isolated directory batches, strict 50:50 comparisons, design-only outputs, and wallpapers."
---
...
- The canonical presentation is a 3:4 portrait canvas with reality above and the lifestyle doodle transformation below, exactly 50:50.
...
Write final PNGs directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-121/` or the explicit output root. Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.

Inspect every result at full and thumbnail size. Accept only when the source, ratio, and source visibility are correct; for comparison modes, the split direction and exact 50:50 midpoint are correct; the transformed region follows the lifestyle doodle and mature image–text composition brief directly from the current source; the recognisable subject and minimal handwritten copy share an intentional invisible grid with exceptionally abundant whitespace; text follows the chosen mode and locale; and there is no watermark, SVG substitute, UI, third band, or second-pass artefact.
...
- `references/xxd-panel-121-prompt.zh-CN.md` and `.en.md` — delivery adapter notes
