...
아래 샘플은 서로 다른 원본 참고 이미지를 사용했습니다. Panel 121이 각 이미지를 독립적으로 한 번만 생성했으며 AI 메타데이터도 제거했습니다. 가로 샘플은 왼쪽 원본·오른쪽 디자인의 정확한 50:50, 세로 샘플은 위 원본·아래 디자인의 정확한 50:50입니다.
...
```text
피사체와 서사 이해 → 배경 대부분 제거 → 편안한 마커풍 형태로 단순화 → 크기·자르기 재구성 → 보이지 않는 그리드와 읽기 경로 → 소량의 손글씨 배치 → 많은 여백으로 리듬 완성
```

## 완성작의 식별 특징

- 사진 영역은 정체성, 구조, 자세, 자연광과 색 분위기를 유지하며 미묘한 색 보정만 합니다.
- 기호화된 형태에 조금 떨리고 끊기는 불완전한 선을 사용하고, 색의 넘침·종이 노출·수작업 오차를 허용합니다.
- 완전한 원근법과 정밀한 사실성 대신 배경과 무관한 물체를 적극적으로 덜어냅니다.
- 작은 피사체를 비중앙, 가장자리, 떠 있는 위치 또는 일부 잘린 상태로 배치하며 여백이 양·음의 형태와 거리를 만듭니다.
- 그림과 글은 한 구도입니다. 손글씨의 폭과 필압은 조금 변하지만 자간과 행간은 편집으로 제어합니다.
- 부드럽고 밝은 제한 색은 원본에서 가져오고, 따뜻한 흰색이나 매우 옅은 혼합색을 넓은 호흡 공간으로 씁니다.
